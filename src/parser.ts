...
export const PSEUDO_STATES = ['focus', 'hover', 'active', 'indeterminate', 'checked', 'visited', 'disabled', 'default'];
export const PSEUDO_ELEMENTS = ['before', 'after', 'selection', 'placeholder', 'marker'];
export const MOZ_PSEUDO_STATES = ['-moz-focusring'];
export const RESTRICTED_SPEC_KEYS: Array<keyof LoadedDesignSpec> = ['extendedSpecs', 'includedSpecs'];
      ...
      if (typeof token.value === 'object') {
        for (const [key, value] of Object.entries(token.value)) {
          ...
          if (inherit && parent?.value) {
            const source: TokenValue =
              typeof (parent.value as string) === 'string'
                ? parent.value
        ...
        if (inherit && parent?.value) {
          const source: TokenValue =
            typeof (parent.value as string) === 'string' ? parent.value : ((parent.value['@' as never] || '') as never);
...
export function createDesignMap(
  ...
  (spec.designs || []).forEach((design, i) => {
    ...
    if (typeof design.variables === 'object' && Object.keys(design.variables ?? {}).length) {
      ...
      design.rules = { ...variables, ...design.rules } as never;
    }
    ...
    if (Object.keys(design.rules || {}).length) {
      ...
      root[name] = {
        ...
        root: design.root === true,
        important: design.important === true,
        mediaVariants: design.mediaVariants ?? [],
        mediaVariables: design.mediaVariables ?? true,
        name: design.name,
    ...
    if (typeof value === 'string' || Array.isArray(value)) {
      ...
      for (const [name, values] of Object.entries(value)) {
        ...
        for (const name of names) {
          ...
          } else if (name.startsWith(':') || name.startsWith(CHILD_SELECTOR)) {
...
function parsePseudoVariants(name: string): PseudoVariant[] {
  ...
  if (PSEUDO_STATES.includes(alias)) {
    return [
      {
        name: `:${alias}`,
        type: 'pseudo-state',
  ...
  } else if (PSEUDO_ELEMENTS.includes(alias) || MOZ_PSEUDO_STATES.includes(alias)) {
    return [
      {
        name: `::${alias}`,
        type: 'pseudo-element',
  ...
  } else if (name.startsWith(CHILD_SELECTOR)) {
    return [
      {
        name: `${name.replace(CHILD_SELECTOR, ' ').replace(/^\s\s/, ' ')}`,
        type: 'element',
      },
    ];
  } else {
