...
import { DesignSpec, DesignToken, getToken, TokenType } from '../../token.js';
...
function parseToken(
  ...
  if (typeof token.value === 'string') {
    root[name] = parseTokenValue(spec, name, token.value, token.type, prefix);
  } else if (typeof token.value === 'object') {
    for (const [ key, value ] of Object.entries(token.value)) {
      if (key.startsWith('@') || key === '.') {
        if (key === '@' || key === '.') {
          root[name] = parseTokenValue(spec, name, value as string, token.type, prefix);
        } else {
          ...
          if (customs) {
            ...
            if (newQuery) {
              ...
              queries[q][custom][name as never] = parseTokenValue(
                spec,
                name,
                value as string,
                token.type,
              ...
              queries[custom][name] = parseTokenValue(spec, name, value as string, token.type, prefix) as never;
            }
          } else {
            const q = `@media ${ query }`;
            ...
            queries[q].root[name] = parseTokenValue(spec, name, value as string, token.type, prefix);
          }
        }
      }
    }
  ...
  if (token.tokens) {
    for (const child of token.tokens) {
      const { root: childRoot, queries: childQueries } = parseToken(spec, child, name, prefix);
...
function parseTokenValue(spec: DesignSpec, name: string, value: string, kind?: TokenType, prefix?: string): string {
  return resolveCssValue(spec, value, prefix, name, kind);
}
